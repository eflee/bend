...
class TestReadmeExamples:
    ...
    def test_sorting_and_limiting(self):
        ...
        df = pd.DataFrame(
            {"name": ["Alice", "Bob", "Charlie", "Diana"], "revenue": [100, 400, 200, 300]}
        )
        q = Q(df)
    ...
    def test_map_examples(self):
        ...
        # Combine names
        q2 = q.map(lambda x: {"full_name": f"{x.first} {x.last}", "email": x.email})
    ...
    def test_groupby_examples(self):
        ...
        df = pd.DataFrame(
            {"region": ["East", "West", "East", "West"], "amount": [100, 200, 150, 250]}
        )
        q = Q(df)
    ...
    def test_column_visibility(self):
        ...
        df = pd.DataFrame(
            {"id": [1, 2], "name": ["Alice", "Bob"], "cost": [50, 75], "revenue": [100, 150]}
        )
        q = Q(df)
    ...
    def test_real_world_pipeline(self):
        ...
        result = (
            q.assign(total=lambda x: x.price * x.quantity)
    ...
    def test_validation_pipeline(self):
        ...
        clean = (
            q.filter(lambda x: not x.email.endswith("@test.com"))
    ...
    def test_export(self, tmp_path):
        ...
        df = pd.DataFrame({"category": ["A", "B", "A"], "price": [10, 20, 15], "qty": [2, 3, 4]})
        q = Q(df)
    ...
    def test_change_history_inspection(self):
        ...
        df = pd.DataFrame({"price": [100, 200], "cost": [60, 120], "revenue": [200, 400]})
        q = Q(df)

        q2 = (
            q.assign(margin=lambda x: (x.price - x.cost) / x.price)
