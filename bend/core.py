...
def _load_csv_to_dataframe(
    path_or_url: str, skip_rows: int = 0, dtype: dict = None
) -> pd.DataFrame:
...
class Q:
    ...
    def _apply_changes(
        self, base: pd.DataFrame, changes: List[Tuple[str, Any]] = None
    ) -> pd.DataFrame:
        ...
        for change_type, change_data in changes_to_apply:
            if change_type == "assign":
                ...
                result = result.sort_values(
                    list(cols) or result.columns.tolist(), ascending=ascending
                )
                ...
                result = result.sample(
                    n=params["n"], frac=params["frac"], random_state=params["random_state"]
                )
    ...
    def merge(
        self, other: "Q", on, how: str = "inner", resolve: dict = None, deep_copy: bool = True
    ) -> "Q":
    ...
    def reload(self, allow_partial_reload: bool = False) -> "Q":
        ...
        if not self._source_path:
            if not allow_partial_reload:
                raise ValueError(
                    "Cannot reload: no source path available. Use allow_partial_reload=True to skip."
                )
            else:
